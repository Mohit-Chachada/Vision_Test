...
bool Num_Extract::validate (Mat mask, Mat pre){
    ...
    ss << img_no;
    string s_no = ss.str();
    //imwrite("/home/mohit/Pictures/training_samples/"+s_no+"_2.png",mask);
    //imwrite("/home/mohit/Pictures/training_samples/"+s_no+"_3.png",pre);
...
vector<Mat> Num_Extract::rotate_bins(vector<Mat>masked,Mat pre){
    vector<Mat> dst;
    Scalar color = Scalar(255,255,255);

    Mat grey,grey0,grey1,grey2,grey3;
    vector<Vec4i> hierarchy;
    std::vector<std::vector<cv::Point> > ext_contour,contour;
    RotatedRect outrect;
...
// new (faster) extract_number() 
	Mat final_num = Mat::zeros(Size(crop_img.cols,crop_img.rows/2),CV_8UC3);	
	//if(!crop_img.empty()&&crop_img.rows>2) 
	{       
		Mat hsv0_thresh,crop_grey;
		vector<Mat> hsv_crop;
		//        cvtColor(crop_img,crop_grey,CV_BGR2HSV);
		//        split(crop_grey,hsv_crop);
		split(crop_img,hsv_crop);
		
		equalizeHist( hsv_crop[2], hsv_crop[2] ); 
		normalize(hsv_crop[2], hsv_crop[2], 0, 255, NORM_MINMAX, CV_8UC1);

		cv::threshold(hsv_crop[2], hsv0_thresh, 0, 255, CV_THRESH_BINARY|CV_THRESH_OTSU);
		//imshow("otsu adaptive thresh",hsv0_thresh);

		Mat hsv0_thresh_tophalf = Mat::zeros(Size(hsv0_thresh.cols,hsv0_thresh.rows/2),CV_8UC1);
		int kktop=0;
		for(int v=0;v<hsv0_thresh.rows;v++){
		     for(int u=0;u<hsv0_thresh.cols;u++){
			if (v<hsv0_thresh.rows/2){
				hsv0_thresh_tophalf.data[kktop]=hsv0_thresh.at<uchar>(v,u);
				kktop=kktop+1;
			}
		     }
		 }
		//imshow("otsu adaptive thresh_tophalf",hsv0_thresh_tophalf);

		Mat hsv0_thresh_bothalf = Mat::zeros(Size(hsv0_thresh.cols,hsv0_thresh.rows/2),CV_8UC1);
		int kkbot=0;
		for(int v=0;v<hsv0_thresh.rows;v++){
		    for(int u=0;u<hsv0_thresh.cols;u++){
			if (v>hsv0_thresh.rows/2){
				hsv0_thresh_bothalf.data[kkbot]=hsv0_thresh.at<uchar>(v,u);
				kkbot=kkbot+1;
			}
		    }
		}
		//imshow("otsu adaptive thresh_bothalf",hsv0_thresh_bothalf);

		int whiteCount_top=cv::countNonZero(hsv0_thresh_tophalf);
		int whiteCount_bot=cv::countNonZero(hsv0_thresh_bothalf);
		//cout<<"int t "<<whiteCount_top<<endl;
		//cout<<"int b "<<whiteCount_bot<<endl;

		if(whiteCount_top>whiteCount_bot){
			//top region is number => Rotate and then take bottom part
		...
		int kk_rot=0;
		for(int v=0;v<crop_img.rows;v++){
		    for(int u=0;u<crop_img.cols;u++){
			if (v>crop_img.rows/2){
				final_num.data[kk_rot]=crop_img.at<Vec3b>(v,u)[0];
				final_num.data[kk_rot+1]=crop_img.at<Vec3b>(v,u)[1];
				final_num.data[kk_rot+2]=crop_img.at<Vec3b>(v,u)[2];
			kk_rot=kk_rot+3;
			}
		    }
		}

		imshow("extracted no",final_num);
		//if(final_num.rows>1 && final_num.cols>1)
		dst.push_back(final_num);//building output list
	}		
...
cout<<"aspect ratio is :"<<aspect_ratio<<endl;*/
            ...
            if(aspect_ratio>1.0 && aspect_ratio<1.4 && ratio>0.25 && ratio<0.55){//test_Jan15
                goodBoxFound = true;
            }
...
cout<<"bad box after good : "<<badBoxAfterGood<<endl;*/
        ...
        time=clock()-time;
        //cout<<"multi time "<<(float (time)/CLOCKS_PER_SEC)<<"\n";
...
//    adaptiveThreshold(grey_img, grey_img, 255, ADAPTIVE_THRESH_MEAN_C, THRESH_BINARY_INV,15, 0);
    ...
    testData.open((_ros_path +"/src/lib/number_detect/test_data").c_str());
    testData.clear();
    ...
    //test_Jan15: ros::package::getPath is taking most of the total execution time
    ...
    sprintf(Tscale_argv[2], "%s", (_ros_path+"/src/lib/number_detect/training_data.range").c_str());
    //sprintf(Tscale_argv[3], "%s", (ros::package::getPath("auv_vision")+"/src/lib/number_detect/test_data").c_str());
    sprintf(Tscale_argv[3], "%s", (_ros_path+"/src/lib/number_detect/test_data").c_str());
    char* TscaleOP;
    TscaleOP = new char [200];
    //sprintf(TscaleOP, "%s", (ros::package::getPath("auv_vision")+"/src/lib/number_detect/test_data.scale").c_str());
    sprintf(TscaleOP, "%s", (_ros_path+"/src/lib/number_detect/test_data.scale").c_str());
...
//        vector<Mat> dst = extract_Number(bins,pre);
	  vector<Mat> dst = rotate_bins(bins,pre);
        timer=clock()-timer;
        //cout<<"extract_Number time "<<(float (timer)/CLOCKS_PER_SEC)<<"\n";
        numbers_found = true;
        ...
        if(numbers_found){
            ...
            const char* modelName = (_ros_path+"/src/lib/number_detect/training_data.model").c_str();
            ...
            //cout<<"svm done\n";
            timer=clock()-timer;
            //cout<<"predict_Number time "<<(float (timer)/CLOCKS_PER_SEC)<<"\n";
