...
void storeDescriptionFiles()
{
    ...
    for (int pos_no = 0; pos_no < POS_SAMPLES_NO; pos_no++)
    {
            ...
            pos_info<<"pos/"<<pos_no<<".png"<<" 1 0 0 50 10\n";
    }
    pos_info.close();
...
int main (int argc, char** argv) {
    ...
    char command[1000];
    sprintf(command,"opencv_createsamples -vec parking.vec -info pos.info -w 50 -h 10 -num 2100");  // -show
    system(command);
    sprintf(command,"opencv_traincascade -data data -vec parking.vec -bg bg.txt -numPos 2000 -numNeg 3000 -numStages 7 -featureType LBP -w 50 -h 10 -bt GAB");
    system(command);
