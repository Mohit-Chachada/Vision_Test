clear;
clc;
pts1 = dlmread('pts1_homography.txt');
pts2 = dlmread('pts2_homography.txt');
...
for count=1:2
    ...
    for i=1:2:N
        A(i,:) = [X1(i) Y1(i) 1 0 0 0 -X2(i)*X1(i) -X2(i)*Y1(i) -X2(i)];
        A(i+1,:) = [0 0 0 X1(i) Y1(i) 1 -Y2(i)*X1(i) -Y2(i)*Y1(i) -Y2(i)];
    end
    ...
    H12 = [t(1) t(2) t(3); t(4) t(5) t(6); t(7) t(8) t(9)];
    %disp (H12);
    
    gamma = 0;
    for i=1:N
        gamma = gamma + 1/(H12(3,1)*X1(i)+H12(3,2)*Y1(i)+H12(3,3));
    end
    gamma = gamma/N;
    H12 = gamma*H12;
    if(count==1)
